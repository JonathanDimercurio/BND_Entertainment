...
export const restrictBoundary = (
  ...
  if (rect.y + translate.y <= boundaryRect.y) {
    ...
    rect.y + rect.height + translate.y >=
    boundaryRect.y + boundaryRect.height
  ) {
    newTranslate.y =
      boundaryRect.y + boundaryRect.height - (rect.y + rect.height);
  }

  if (rect.x + translate.x <= boundaryRect.x) {
    newTranslate.x = boundaryRect.x - rect.x;
  } else if (
    rect.x + rect.width + translate.x >=
    boundaryRect.x + boundaryRect.width
  ) {
    newTranslate.x =
      boundaryRect.x + boundaryRect.width - (rect.x + rect.width);
  }
